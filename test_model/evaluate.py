...
def generate(datazet, nr):
    ...
    for ik, each in enumerate(datazet):
        temp1, masslossrate1, massloss1 = calculate_reaction(np.exp(each[0:reactions]), each[3:6], each[6:9], HR=5.,
                                                             dt=24)
        temp2, masslossrate2, massloss2 = calculate_reaction(np.exp(each[0:reactions]), each[3:6], each[6:9], HR=10.,
                                                             dt=12)
        temp3, masslossrate3, massloss3 = calculate_reaction(np.exp(each[0:reactions]), each[3:6], each[6:9], HR=30.,
                                                             dt=4)
        temp4, masslossrate4, massloss4 = calculate_reaction(np.exp(each[0:reactions]), each[3:6], each[6:9], HR=40.,
                                                             dt=3)
        return_list[
            len(datazet) * nr + ik] = temp1, temp2, temp3, temp4, masslossrate1, masslossrate2, masslossrate3, masslossrate4, massloss1, massloss2, massloss3, massloss4
...
def generate_2(datazet, labels, nr):
    ...
    for ik, each in enumerate(datazet):
        ...
        minim2 = []
        for eachs in feat3:
            minim2.append(np.sqrt(
                mean_squared_error(np.array(feat1[0, 4:8]).reshape(1, -1), np.array(eachs[4:8]).reshape(1, -1))) / (
                                      np.max(np.array(feat1[0, 4:8]).reshape(1, -1)) - np.min(
                                  np.array(feat1[0, 4:8]).reshape(1, -1))))
...
def generate_3(dataset, nr):
    ...
    for ik, each in enumerate(np.copy(dataset)):
        ...
        feat.append((temp1, temp2, temp3, temp4, masslossrate1, masslossrate2, masslossrate3, masslossrate4, massloss1,
                     massloss2, massloss3, massloss4))
    scale = len(temp1)
    feat = np.array(feat)
    return feat
...
n2.append(
    r2_score(complete_labels[50000:100000, [0, 1, 3, 4, 6, 7]], complete_prediction[50000:100000, [0, 1, 3, 4, 6, 7]]))
...
if __name__ == '__main__':
    ...
    for n in range(corez):
        p = Process(target=generate,
                    args=(complete_prediction[int((n * (k / corez))):int(((k / corez) + n * (k / corez)))], n,))
        processes.append(p)
...
for it, each2 in enumerate(np.copy(features)):
    dataset.append(np.sqrt(mean_squared_error(features[it], prediction_features[it])) / (
                np.max(features[it]) - np.min(features[it])))
...
if __name__ == '__main__':
    ...
    for n in range(corez):
        p = Process(target=generate_2, args=(r2_prediction[int((n * (k / corez))):int(((k / corez) + n * (k / corez)))],
                                             r2_labels[int((n * (k / corez))):int(((k / corez) + n * (k / corez)))],
                                             n,))
        processes.append(p)
...
if __name__ == '__main__':
    ...
    return_prediction = manager.list([0] * k)
    for n in range(corez):
        p = Process(target=generate_2, args=(r3_prediction[int((n * (k / corez))):int(((k / corez) + n * (k / corez)))],
                                             r3_labels[int((n * (k / corez))):int(((k / corez) + n * (k / corez)))],
                                             n,))
        processes.append(p)
...
n, bins, patches = ax0.hist(
    (complete_histogram2[0:50000], complete_histogram2[50000:100000], complete_histogram2[100000:150000]), bins=100,
...
n, bins, patches = ax1.hist(complete_histogram2, bins=1000, range=(0, 0.251), weights=weights, cumulative=True,
