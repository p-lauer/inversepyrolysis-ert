...
def generate(dataset,nr):
    ...
    feat=[]
    np.savetxt("./labels{}k_5_10_30_40_3r_2ks_{}.csv".format(int(i/1000),int(nr)), np.copy(dataset.reshape(int((i/cores)*a),18)), delimiter=",")
    ...
    np.savetxt("./features{}k_5_10_30_40_3r_2ks_{}.csv".format(int(i/1000),nr), (np.copy(feat[:,4:8])).reshape(int(i/cores),scale*4), delimiter=",")
...
# RR/RT for 3 components
a=1
dataset=[]
for each in range(i):
    ia=round(uniform(0,1),4)
    ib=round(uniform(0,1-ia),4)
    ic=1-ia-ib
    ...
    RR3,RT3=round(uniform(rrlimlow,rrlimup),6),randrange(rtlimlow,rtlimup)
    E,A=[0,0,0],[0,0,0]
    A[0],E[0]=convert_tga_to_rk(RR,RT,5,ia)
    A[1],E[1]=convert_tga_to_rk(RR2,RT2,5,ib)
    A[2],E[2]=convert_tga_to_rk(RR3,RT3,5,ib)
    dataset.append(([RR,RR2,RR3],[RT,RT2,RT3],[ia,ib,ic],A,E,[ia,ib,ic]))

starttime=time.time()
dataset=np.array(dataset)
np.savetxt("./labels{}k_5_10_40_3r_2ks.csv".format(int(i/1000)), np.copy(dataset.reshape(i*a,18)), delimiter=",")
